  ...
  if( Serial.available() > inpLen - 1 ){
    ...
    while( Serial.available() ){
      ...
      if( readByte == SOP ){
        ...
        // 5. else no joy , erase the input arr
        }else{
          gotBytes = false;
          zero_input();
          digitalWrite(LED_BUILTIN, LOW);
...
//      if( readByte == SOP ){
//        input[0] = readByte;
//        for( int i = 1 ; i < outpLn ; i++ ){ outpt[i] = Sensor.read(); } // Read 4 bytes from the serial connection
//        // 4. If the packet ends properly , then set joy flag
//        if( outpt[outpLn - 1] == EOP ){
//          gotBytes = true;
//        // 5. else no joy , erase the input arr
//        }else{
//          gotBytes = false;
...
//    for( byte j = 0 ; j < _REPEAT ; j++ ){
////      for( byte i = 0 ; i < inpLen ; i++ ){ Sensor.write( input[i] ); } // Write 4 bytes to the serial connection
//      Serial.write( (byte*)outpt , sizeof( outpt ) );
//    }
//  }
